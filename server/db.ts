...
const DATA_FILE = path.join(DATA_DIR, 'kisaansathi.json');
...
const INITIAL_TOKENS: ProcurementToken[] = [];
const INITIAL_WEIGHINGS: WeighingRecord[] = [];
const INITIAL_QUALITY: QualityCheck[] = [];
...
class Database {
  ...
  private loadData(): DatabaseSchema {
    try {
      ...
      console.warn('Could not read persistent file, bootstrapping fresh data:', err);
    }
  ...
  // Hydrate token with related entities
  ...
  public getFarmerByMobile(mobile: string): Farmer | undefined {
    ...
    return this.data.farmers.find(f => f.mobile_number.slice(-10) === clean);
  }
  ...
  public registerFarmer(payload: {
    ...
    const aadharLast4 = payload.aadhar_last4 || (cleanAadhar ? cleanAadhar.slice(-4) : Math.floor(1000 + Math.random() * 9000).toString());
  ...
  public createStaff(payload: Partial<Staff> & { name: string; username: string; procurement_center_id: string; role: Staff['role'] }): Staff {
  ...
  public resetStaffPassword(id: string, customPassword?: string): { staff: Staff; new_password: string } | null {
  ...
  public getAdminById(id: string): Admin | undefined {
    return this.data.admins.find(a => a.id === id);
  }

  public getAdminByUsername(username: string): Admin | undefined {
  ...
  public updateCenter(id: string, updates: Partial<ProcurementCenter>): ProcurementCenter | null {
  ...
  public getTokenById(id: string): ProcurementToken | null {
  ...
  public bookToken(payload: {
    ...
    const farmer = this.getFarmerById(payload.farmer_id);

    // Compute token number: KS-{Year}-{CenterCode}-{RandomNumber}
    ...
    // Compute queue number for that center and date
  ...
  public updateTokenStatus(tokenId: string, status: ProcurementToken['status']): ProcurementToken | null {
    const token = this.data.tokens.find(t => t.id === tokenId);
    if (!token) return null;
    token.status = status;

    // Send notification on milestone changes
  ...
  public recordQualityCheck(payload: {
    ...
    // Standard Mandi / FCI Moisture logic:
    // If moisture <= maxMoisture: 100% full pass, Grade A
    // If moisture > maxMoisture and <= maxMoisture + 2.0: FAQ with refraction deduction
    // If moisture > maxMoisture + 2.5: Rejected (Too wet / fungal risk)
    ...
    // Deduction calculation: ₹ per quintal based on slabs
  ...
  // --- Final Acceptance & Payment Generation ---
  public acceptProcurement(tokenId: string): { token: ProcurementToken; payment: PaymentRecord } | null {
    const token = this.data.tokens.find(t => t.id === tokenId);
    if (!token) return null;
  ...
  // --- Process Payment / Settle UTR ---
  public processPayment(paymentId: string, utr?: string): PaymentRecord | null {
  ...
  public getStats(): ProcurementStats {
    ...
    // Unique farmers who have booked tokens
    ...
    return {
      ...
      tokens_today: this.data.tokens.filter(t => t.booking_date === new Date().toISOString().split('T')[0] || true).length,
      ...
      average_waiting_time_mins: 28, // drastically reduced due to slot booking
